...
const Rows = (movieData, index, admin) => {
    ...
    return (
        <tr key={index} onClick={() => navigate(`/movie/${movie.id}/${movie.title}`)} className={'cursor-pointer'}>
            ...
            <td className={`${Text} justify-between h-full`}>
                <button className="text-white text-lg rounded flex-colo w-6 h-6 hover:text-subMain m-2">
                    <AiOutlineClose/>
                </button>

                {admin && (
                    <>
                        <button className="borders border-border bg-dry flex-rows gap-2 text-border rounded py-1 px-2">
