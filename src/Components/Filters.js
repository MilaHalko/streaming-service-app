...
function Filters({handleFilter}) {
    ...
    return (
        <div
            className={`bg-dry border text-dryGray border-border md:grid md:grid-cols-${Filter.length + 1} gap-6 rounded-lg p-4`}>
            {
                Filter.map((item, index) => (
                    <Listbox key={index} value={item.value} onChange={item.onchange}>
                        <div className="relative">
                            ...
                            {/* Filter title */}
                            <Listbox.Button
                                className="relative border border-border w-full text-white bg-main rounded-lg cursor-default py-4 pl-6 pr-10 text-left text-xs mb-2 md:mb-0">
